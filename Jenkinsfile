...
pipeline {    
    ...
    stages {
        ...
        stage('Integration Test') {
            steps {
                sh "docker exec apm-nagios-test-${BUILD_NUMBER} mvn clean verify -DskipDocker=true"
            }
        }
        stage('Deploy') {
	    when {
            ...
            steps {
		sh "mvn deploy -P buildDockerImageOnJenkins -DdockerImage.tag=application-performance-monitoring-develop.${env.BUILD_NUMBER} -Ddocker.registry=docker-dev-local.art.local -DdeleteDockerImages=true -DskipTests=true -DskipITs"
            }
        }
                ...
                dir('/opt'){
                    ...
                    sh '''
                        github-release release \
                            ...
                            --repo  apm-nagios-parent \
                            --tag v0.0.1-${BRANCH_NAME}-${BUILD_ID} \
                            --name "apm-nagios-parent release" \
                            --description "apm-nagios-parent release"
                        github-release upload \
                            --user dellemc-symphony \
                            --repo apm-nagios-parent \
                            --tag v0.0.1-${BRANCH_NAME}-${BUILD_ID} \
                            --name "apm-nagios-parent release" \
                            --file ${WORKSPACE}/target/apm-nagios-parent-1.0-SNAPSHOT.jar
                    '''
                }
            }
        }
	    ...
	    when {
            ...
            steps {
                ...
                sh "sh /opt/nexB/scancode --format html ${WORKSPACE} /opt/nexB/nexb-output/apm-nagios-parent.html"
                sh "sh /opt/nexB/scancode --format html-app ${WORKSPACE} /opt/nexB/nexb-output/apm-nagios-parent-grap.html"	       
