...
SERVICE_BUILD_ROOT=${RPM_BUILD_ROOT}/opt/dell/apm-nagios
...
cp -rf ${RPM_SOURCE_DIR}/docker/compose/systemd/apm-nagios.service ${RPM_BUILD_ROOT}/usr/lib/systemd/system/apm-nagios.service
...
%post
if [ $1 -eq 1 ];then
    /bin/sh /opt/dell/apm-nagios/install/apm-nagios-install.sh
elif [ $1 -eq 2 ];then
    /bin/sh /opt/dell/apm-nagios/install/apm-nagios-upgrade.sh
else
...
%preun
if [ $1 -eq 0 ];then
    /bin/sh /opt/dell/apm-nagios/install/apm-nagios-remove.sh
fi
exit 0
...
%attr(644,root,root) /usr/lib/systemd/system/apm-nagios.service

%attr(0754,rcm,dell) /opt/dell/apm-nagios/
%attr(0754,rcm,dell) /opt/dell/apm-nagios/install/
